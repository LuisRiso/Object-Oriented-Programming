...
if __name__ == "__main__":
    ...
    dadosPessoasCadastro = [
        ...
        ("Aluno", "Beatriz", "Av. Oeste, 909", 18, "45612378901", "SIN")            # CPF Invalido
    ]

    for tipo, nome, endereco, idade, cpf, infoExtra in dadosPessoasCadastro:
        
        try:
            if tipo == "Professor":
                if infoExtra.lower() != "doutor":
                    raise titulacaoInvalida("Erro: Professor(a) {} possui titulação abaixo da estipulada, necessário ser um Doutor".format(nome))
                if idade < 30:
                    raise idadeInvalida(("Erro: Professor(a) {} deve ter idade igual ou maior que 30 anos.".format(nome)))
                pessoa = Professor(nome, endereco, idade, cpf, infoExtra)

            elif tipo == "Aluno":
                if infoExtra.upper() not in ["CCO", "SIN"]:
                    raise cursoInvalido("Erro: Aluno {} precisa estar cadastro em SIN ou CCO".format(nome))
                if idade < 18:
                    raise idadeInvalida(("Erro: Aluno(a) {} deve ter idade igual ou maior que 18 anos.".format(nome)))
                pessoa = Aluno(nome, endereco, idade, cpf, infoExtra)

            if cpf in cpfsCadastrados:
                raise erroCPF("CPF Invalido para {} pois já está cadastrado no sistema".format(nome))
