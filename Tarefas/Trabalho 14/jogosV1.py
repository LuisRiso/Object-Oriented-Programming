...
class Jogo():
    def __init__(self, controle, codigo, titulo, console, genero, preco, notaAvaliacoes):
        self.controleJogo = controle
        self.codigo = codigo
        self.titulo = titulo
    ...
    def codigo(self, valor):
        for jogo in self.controleJogo.listaJogosCadastrados:
            if jogo.codigo == valor:
                raise ValueError("Codigo já cadastrado: {}".format(valor))
        if valor == "":
    ...
    def titulo(self, valor):
        for jogo in self.controleJogo.listaJogosCadastrados:
            if jogo.titulo == valor:
                raise ValueError("Titulo já cadastrado: {}".format(valor))
        if valor == "":
...
class CtrlJogo():
    ...
    def salvarJogos(self):
        ...
        # Remove o atributo 'controleJogo' temporariamente
            for jogo in self.listaJogosCadastrados:
                if hasattr(jogo, "controleJogo"):
                    delattr(jogo, "controleJogo")
        # Salva a lista de jogos
    ...
    def cadastraHandler(self, event):
        ...
        preco = float(self.limiteInsercaoJogo.inputPreco.get())
        try:
            jogo = Jogo(self, codigo, titulo, console, genero, preco, int(1))
