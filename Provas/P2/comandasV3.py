...
class LimiteCalculoFaturamento():
    def __init__(self, textoFaturamento):
        messagebox.showinfo('Lista de Cupons', textoFaturamento)
...
class CtrlComanda():
            ...
            if buscaComanda.nroComanda == nroComanda:
                for product in listaProdutos:
                    if produto == product.descricao:
                        if product.codigo not in buscaComanda.produtosComanda: #alterou o product para product.codigo
                            buscaComanda.produtosComanda[product.codigo] = {
                                    "descricao": product.descricao,
                                    "quantidadeProduto": int(quantidadeProduto),
                                    "precoTotalProduto": float(product.valorUnitario) * int(quantidadeProduto) 
                                }
                        else:
    ...
    def calcularFaturamento(self):
        textoFaturamento = "Faturamento:\n"
        valorTotalRefeicao = 0
        valorTotalProduto = 0
        valorTotalFaturado = 0
        for comanda in self.listaComandasCriadas:
            for codigoProduto, dadosProduto in comanda.produtosComanda.items():
                if codigoProduto == 901 or codigoProduto == 902:
                    valorTotalRefeicao += float(dadosProduto["precoTotalProduto"])
                else:
                    valorTotalProduto += float(dadosProduto["precoTotalProduto"])
                valorTotalFaturado += float(dadosProduto["precoTotalProduto"])
        textoFaturamento += "Faturamento Refeição: R$ " + str(valorTotalRefeicao) + '\n'
        textoFaturamento += "Faturamento Produtos: R$ " + str(valorTotalProduto) + '\n'
        textoFaturamento += "Faturamento Total: R$ " + str(valorTotalFaturado) + '\n'
        self.limiteFaturamento = LimiteCalculoFaturamento(textoFaturamento)
        return
