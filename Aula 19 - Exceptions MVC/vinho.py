...
class Vinho:
    def __init__(self, codigo, nome, tipo, variedade, origem, preco):
        ...
        self.__nome = nome
        self.tipo = tipo #vai validar essa variavel
        self.variedade = variedade #vai validar essa variavel
        self.origem = origem #vai validar essa variavel
        self.preco = preco #vai validar essa variavel
...
class CtrlVinho():
    ...
    def consultaVinho(self):
        ...
        for vinho in self.listaVinhos:
            #se não está na lista de tipos self.tipos, cadastra o tipo na lista 
            #apenas tipos já cadastrados em algum vinho
    ...
    def exibeTipo(self,event):
        self.limiteCons.comboboxVariedade.set(value='---')
        tipoSel = self.limiteCons.comboboxTipo.get()
        self.limiteCons.textVinhos.config(state='normal') #state='normal' -> habilita a caixa de texto para escrita -> vem por padrão desabilitada
        self.limiteCons.textVinhos.delete(index1='1.0', index2=tk.END) #vai deletar o que tiver na caixa de texto
        for vinho in self.listaVinhos:
            if vinho.tipo == tipoSel:
                vinhoSel = vinho.getVinho() + '\n\n'
                self.limiteCons.textVinhos.insert(index=1.0, chars=vinhoSel) #vai inserir o texto dos vinhos para a caixa de texto
        self.limiteCons.textVinhos.config(state='disabled')

    def exibeVariedade(self, event):
        self.limiteCons.comboboxTipo.set(value='---')
